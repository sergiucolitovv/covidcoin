...
#include "zcov19/accumulatorcheckpoints.h"
...
CzCOV19Wallet* zwalletMain = NULL;
...
void Shutdown()
{
    ...
    if (nLocalServices == NODE_BLOOM_LIGHT_ZC) {
        lightWorker.StopLightZcov19Thread();
    }
#ifdef ENABLE_WALLET
    delete pwalletMain;
    pwalletMain = NULL;
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the COV19 and zCOV19 money supply statistics") + " " + _("on startup"));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in COV19/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in COV19/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    std::string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, covid19, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero, staking)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in COV19/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-cov19stake=<n>", strprintf(_("Enable or disable staking functionality for COV19 inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zcov19stake=<n>", strprintf(_("Enable or disable staking functionality for zCOV19 inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-backupzcov19=<n>", strprintf(_("Enable automatic wallet backups triggered after each zCOV19 minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zcov19backuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zCOV19 backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2015-%i The COVID19 Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
        ...
        do {
            ...
            try {
                ...
                if(Params().NetworkID() == CBaseChainParams::MAIN && chainHeight > Params().Zerocoin_Block_EndFakeSerial()) {
                    ...
                    CAmount zcov19SupplyCheckpoint = Params().GetSupplyBeforeFakeSerial() + GetWrapppedSerialInflationAmount();

                    if (pblockindex->GetZerocoinSupply() < zcov19SupplyCheckpoint) {
                        // Trigger reindex due wrapping serials
                        LogPrintf("Current GetZerocoinSupply: %d vs %d\n", pblockindex->GetZerocoinSupply()/COIN , zcov19SupplyCheckpoint/COIN);
                        reindexDueWrappedSerials = true;
                    } else if (pblockindex->GetZerocoinSupply() > zcov19SupplyCheckpoint) {
                        // Trigger global zCOV19 reindex
                ...
                if (GetBoolArg("-reindexmoneysupply", false) || reindexZerocoin) {
                    ...
                    RecalculateCOV19Supply(reindexZerocoin ? Params().Zerocoin_StartHeight() : 1);
                }
                ...
                if(Params().NetworkID() == CBaseChainParams::MAIN && chainHeight > Params().Zerocoin_Block_EndFakeSerial()) {
                    ...
                    CAmount zcov19SupplyCheckpoint = Params().GetSupplyBeforeFakeSerial() + GetWrapppedSerialInflationAmount();
                    if (pblockindex->GetZerocoinSupply() != zcov19SupplyCheckpoint)
                        return InitError(strprintf("ZerocoinSupply Recalculation failed: %d vs %d", pblockindex->GetZerocoinSupply()/COIN , zcov19SupplyCheckpoint/COIN));
                }
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        zwalletMain = new CzCOV19Wallet(pwalletMain->strWalletFile);
        ...
        //Inititalize zCOV19Wallet
        uiInterface.InitMessage(_("Syncing zCOV19 wallet..."));

        pwalletMain->InitAutoConvertAddresses();

        bool fEnableZCov19Backups = GetBoolArg("-backupzcov19", true);
        pwalletMain->setZCov19AutoBackups(fEnableZCov19Backups);

        //Load zerocoin mint hashes to memory
        pwalletMain->zcov19Tracker->Init();
    ...
    /* Denominations
       ...
       For example:
       1COV19+1000 == (.1COV19+100)*10
       10COV19+10000 == (1COV19+1000)*10
    */
    ...
    if (nLocalServices & NODE_BLOOM_LIGHT_ZC) {
        ...
        lightWorker.StartLightZcov19Thread(threadGroup);
    }
