...
UniValue listunspent(const UniValue& params, bool fHelp)
{
    ...
    if (params.size() > 2) {
        ...
        for (unsigned int inx = 0; inx < inputs.size(); inx++) {
            ...
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid COVID19 address: ") + input.get_str());
...
UniValue createrawtransaction(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw std::runtime_error(
            ...
            "    {\n"
            "      \"address\": x.xxx   (numeric, required) The key is the covid19 address, the value is the covid19 amount\n"
            "      ,...\n"
            "    }\n"
...
UniValue createrawzerocoinstake(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            ...
            "\nCreates raw zCOV19 coinstakes (without MN output). Only for regtest\n" +
            HelpRequiringPassphrase() + "\n"
    ...
    // create the zerocoinmint output (one spent denom + three 1-zCOV19 denom)
    ...
    if (!pwalletMain->CreateZCOV19OutPut(staked_denom, vOutMint[1], dMint))
        throw JSONRPCError(RPC_WALLET_ERROR, "failed to create new zcov19 output");

    for (int i=2; i<5; i++) {
        if (!pwalletMain->CreateZCOV19OutPut(libzerocoin::ZQ_ONE, vOutMint[i], dMint))
            throw JSONRPCError(RPC_WALLET_ERROR, "failed to create new zcov19 output");
    }
    ...
    CTxIn newTxIn;
    // Use v2 spends for input (with v3 zcov19 staking is disabled)
...
UniValue createrawzerocoinspend(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw std::runtime_error(
            ...
            "\nCreates raw zCOV19 public spend.\n" +
            HelpRequiringPassphrase() + "\n"
