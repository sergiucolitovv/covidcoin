...
BOOST_AUTO_TEST_CASE(zerocoin_wrapped_serial_spend_test)
{
    ...
    CzCOV19Wallet *czCOV19Wallet = new CzCOV19Wallet(wallet.strWalletFile);
    ...
    for (unsigned int i = 0; i < TESTS_COINS_TO_ACCUMULATE; i++) {
        ...
        czCOV19Wallet->GenerateDeterministicZCOV19(denom, coin, dMint, true);
        czCOV19Wallet->UpdateCount();
