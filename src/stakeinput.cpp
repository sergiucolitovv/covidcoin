...
CZCov19Stake::CZCov19Stake(const libzerocoin::CoinSpend& spend)
{
...
int CZCov19Stake::GetChecksumHeightFromMint()
{
...
int CZCov19Stake::GetChecksumHeightFromSpend()
{
    return GetChecksumHeight(nChecksum, denom);
}

uint32_t CZCov19Stake::GetChecksum()
{
    return nChecksum;
}

// The zCOV19 block index is the first appearance of the accumulator checksum that was used in the spend
// note that this also means when staking that this checksum should be from a block that is beyond 60 minutes old and
// 100 blocks deep.
CBlockIndex* CZCov19Stake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CZCov19Stake::GetModifier(uint64_t& nStakeModifier)
{
    ...
    // zCOV19 staking is disabled long before block v7 (and checkpoint is not included in blocks since v7)
...
CDataStream CZCov19Stake::GetUniqueness()
{
    //The unique identifier for a zCOV19 is a hash of the serial
    CDataStream ss(SER_GETHASH, 0);
    ss << hashSerial;
    return ss;
}

bool CZCov19Stake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
...
bool CZCov19Stake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
    //Create an output returning the zCOV19 that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZCOV19OutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zCOV19 output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zCOV19", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZCOV19OutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zCOV19 output", __func__);
...
bool CZCov19Stake::GetTxFrom(CTransaction& tx)
{
    return false;
}

bool CZCov19Stake::MarkSpent(CWallet *pwallet, const uint256& txid)
{
    CzCOV19Tracker* zcov19Tracker = pwallet->zcov19Tracker.get();
    CMintMeta meta;
    if (!zcov19Tracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zcov19Tracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}

//!COV19 Stake
bool CCov19Stake::SetInput(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CCov19Stake::GetTxFrom(CTransaction& tx)
{
    tx = txFrom;
    return true;
}

bool CCov19Stake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CCov19Stake::GetValue()
{
    return txFrom.vout[nPosition].nValue;
}

bool CCov19Stake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
bool CCov19Stake::GetModifier(uint64_t& nStakeModifier)
{
...
CDataStream CCov19Stake::GetUniqueness()
{
    //The unique identifier for a COV19 stake is the outpoint
...
CBlockIndex* CCov19Stake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
    uint256 hashBlock = 0;
