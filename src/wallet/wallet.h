...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 COV19 at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zCOV19 send
enum ZerocoinSpendStatus {
    ZCOV19_SPEND_OKAY = 0,                            // No error
    ZCOV19_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZCOV19_WALLET_LOCKED = 2,                         // Wallet was locked
    ZCOV19_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZCOV19_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZCOV19_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZCOV19_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZCOV19_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZCOV19_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZCOV19_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZCOV19_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZCOV19_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZCOV19_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZCOV19_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZCOV19_SPENT_USED_ZCOV19 = 14,                      // Coin has already been spend
    ZCOV19_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZCOV19_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZCov19BackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZCOV19OutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    std::string GetUniqueWalletBackupName(bool fzcov19Auto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzCOV19Tracker> zcov19Tracker;
    ...
    void setZWallet(CzCOV19Wallet* zwallet);
    CzCOV19Wallet* getZWallet();
    bool isZeromintEnabled();
    void setZCov19AutoBackups(bool fEnabled);
    ...
    /// Get 10000 COV19 output and keys which can be used for the Masternode
    ...
    /** zCOV19 reset */
    boost::signals2::signal<void()> NotifyzCOV19Reset;
