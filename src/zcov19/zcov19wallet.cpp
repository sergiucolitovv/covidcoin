// Copyright (c) 2017-2019 The COVID19 developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include "zcov19wallet.h"
#include "main.h"
#include "txdb.h"
...
CzCOV19Wallet::CzCOV19Wallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zcov19 seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZCOV19Seed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZCOV19Seed_deprecated()) {
                    LogPrintf("%s: Updated zCOV19 seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zcov19 seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zcov19 wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzCOV19Wallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZCOV19Count(nCountLastUsed);
    else if (!walletdb.ReadZCOV19Count(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzCOV19Wallet::Lock()
{
    seedMaster = 0;
}

void CzCOV19Wallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzCOV19Wallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZCOV19(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzCOV19Wallet::LoadMintPoolFromDB()
{
...
void CzCOV19Wallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzCOV19Wallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzCOV19Wallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zcov19Tracker->HasPubcoinHash(pMint.first)) {
...
bool CzCOV19Wallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZCOV19(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zcov19Tracker which also adds to database
    pwalletMain->zcov19Tracker->Add(dMint, true);
...
void CzCOV19Wallet::SeedToZCOV19(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzCOV19Wallet::GetZerocoinSeed(uint32_t n)
{
...
void CzCOV19Wallet::UpdateCount()
{
    nCountLastUsed++;
    CWalletDB walletdb(strWalletFile);
    walletdb.WriteZCOV19Count(nCountLastUsed);
...
void CzCOV19Wallet::GenerateDeterministicZCOV19(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzCOV19Wallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzCOV19Wallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzCOV19Wallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
