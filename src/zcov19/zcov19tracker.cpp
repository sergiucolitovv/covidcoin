// Copyright (c) 2018-2020 The COVID19 developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include <zcov19/deterministicmint.h>
#include "zcov19tracker.h"
#include "util.h"
#include "sync.h"
#include "main.h"
#include "txdb.h"
...
CzCOV19Tracker::CzCOV19Tracker(std::string strWalletFile)
{
...
CzCOV19Tracker::~CzCOV19Tracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzCOV19Tracker::Init()
{
...
bool CzCOV19Tracker::Archive(CMintMeta& meta)
{
...
bool CzCOV19Tracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzCOV19Tracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzCOV19Tracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzCOV19Tracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzCOV19Tracker::GetSerialHashes()
{
...
CAmount CzCOV19Tracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzCOV19Tracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzCOV19Tracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzCOV19Tracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzCOV19Tracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzCOV19Tracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzCOV19Tracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzCOV19Tracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzCOV19Tracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzCOV19Tracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzCOV19Tracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzCOV19Wallet* zCOV19Wallet)
{
    bool iszCOV19WalletInitialized = (NULL != zCOV19Wallet);
    CMintMeta meta;
    ...
    if (! iszCOV19WalletInitialized)
        zCOV19Wallet = new CzCOV19Wallet(strWalletFile);
    meta.isSeedCorrect = zCOV19Wallet->CheckSeed(dMint);
    if (! iszCOV19WalletInitialized)
        delete zCOV19Wallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzCOV19Tracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzCOV19Tracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzCOV19Tracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzCOV19Tracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzCOV19Tracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzCOV19Tracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzCOV19Wallet* zCOV19Wallet = new CzCOV19Wallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zCOV19Wallet);
        }
        delete zCOV19Wallet;
        LogPrint("zero", "%s: added %d dzcov19 from DB\n", __func__, listDeterministicDB.size());
    }
