...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
...

    {
        ...
        if (recipients[0].useSwiftTX && total > sporkManager.GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 COV19.").arg(sporkManager.GetSporkValue(SPORK_5_MAX_VALUE)),
...
bool WalletModel::createZcov19Spend(
        CWalletTx &wtxNew,
...
bool WalletModel::convertBackZcov19(
        CAmount value,
        std::vector<CZerocoinMint> &vMintsSelected,
        bool fMintChange,
        bool fMinimizeChange,
...
static void NotifyzCOV19Reset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzCOV19Reset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzCOV19Reset.connect(boost::bind(NotifyzCOV19Reset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzCOV19Reset.disconnect(boost::bind(NotifyzCOV19Reset, this));
...
bool WalletModel::updateAddressBookPurpose(const QString &addressStr, const std::string& purpose) {
    ...
    if (address.IsStakingAddress())
        return error("Invalid COVID19 address, cold staking address");
    CKeyID keyID;
...
bool WalletModel::getKeyId(const CBitcoinAddress& address, CKeyID& keyID) {
    if (!address.IsValid())
        return error("Invalid COVID19 address");

    if (!address.GetKeyID(keyID))
        return error("Unable to get KeyID from COVID19 address");
...
{
    setMints.clear();
    setMints = pwalletMain->zcov19Tracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus, fWrongSeed);
}
