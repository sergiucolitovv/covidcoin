...
#include "qt/covid19/send.h"
#include "qt/covid19/forms/ui_send.h"
#include "qt/covid19/addnewcontactdialog.h"
#include "qt/covid19/qtutils.h"
#include "qt/covid19/sendchangeaddressdialog.h"
#include "qt/covid19/optionbutton.h"
#include "qt/covid19/sendconfirmdialog.h"
#include "qt/covid19/myaddressrow.h"
#include "qt/covid19/guitransactionsutils.h"
...
SendWidget::SendWidget(COVID19GUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    /* Button Group */
    ui->pushLeft->setText("COV19");
    setCssProperty(ui->pushLeft, "btn-check-left");
    ui->pushLeft->setChecked(true);
    ui->pushRight->setText("zCOV19");
    setCssProperty(ui->pushRight, "btn-check-right");

    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (COV19) or private coins (zCOV19)"));
    ...
    /* Address */
    ui->labelSubtitleAddress->setText(tr("COVID19 address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 COV19");
    ...
    setCssProperty(coinIcon, "coin-icon-cov19");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onCOV19Selected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onCOV19Selected(false);});
...
void SendWidget::refreshView(){
    ...
    if(ui->pushLeft->isChecked()){
        btnText = tr("Send COV19");
        ui->pushButtonAddRecipient->setVisible(true);
    }else{
        btnText = tr("Send zCOV19");
...
void SendWidget::refreshAmounts() {
    ...
    bool isZcov19 = ui->pushRight->isChecked();
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();

    ui->labelAmountSend->setText(GUIUtil::formatBalance(total, nDisplayUnit, isZcov19));
    ...
    if (CoinControlDialog::coinControl->HasSelected()){
        ...
        // Wallet's balance
        totalAmount = (isZcov19 ? walletModel->getZerocoinBalance() : walletModel->getBalance()) - total;
...
void SendWidget::onSendClicked(){
    ...
    bool sendCov19 = ui->pushLeft->isChecked();

    // request unlock only if was locked or unlocked for mixing:
    // this way we let users unlock by walletpassphrase or by menu
    // and make many transactions while unlocking through this dialog
    // will call relock
    if(!GUIUtil::requestUnlock(walletModel, sendCov19 ? AskPassphraseDialog::Context::Send_COV19 : AskPassphraseDialog::Context::Send_zCOV19, true)){
        // Unlock wallet was cancelled
        inform(tr("Cannot send, wallet locked"));
        return;
    }

    if((sendCov19) ? send(recipients) : sendZcov19(recipients)) {
...
bool SendWidget::sendZcov19(QList<SendCoinsRecipient> recipients){
    if (!walletModel || !walletModel->getOptionsModel())
        return false;

    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        emit message(tr("Spend Zerocoin"), tr("zCOV19 is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    // use mints from zCOV19 selector if applicable
    std::vector<CMintMeta> vMintsToFetch;
    std::vector<CZerocoinMint> vMintsSelected;
    if (!ZCov19ControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZCov19ControlDialog::GetSelectedMints();
    ...
    if (walletModel->sendZcov19(
        ...
        inform(tr("zCOV19 transaction sent!"));
        ZCov19ControlDialog::setSelectedMints.clear();
        clearAll();
        return true;
    } else {
        QString body;
        if (receipt.GetStatus() == ZCOV19_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zCOV19 require a security level of 100 to successfully spend.");
        } else {
            int nNeededSpends = receipt.GetNeededSpends(); // Number of spends we would need for this transaction
            const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zCOV19 transaction
        ...
        emit message("zCOV19 transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked(){
    if(isCOV19){
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any COV19 to select."));
        }
    }else{
        if (walletModel->getZerocoinBalance() > 0) {
            ZCov19ControlDialog *zCov19Control = new ZCov19ControlDialog(this);
            zCov19Control->setModel(walletModel);
            zCov19Control->exec();
            ui->btnCoinControl->setActive(!ZCov19ControlDialog::setSelectedMints.empty());
            zCov19Control->deleteLater();
        } else {
            inform(tr("You don't have any zCOV19 in your balance to select."));
        }
    }
}
...
void SendWidget::onCOV19Selected(bool _isCOV19){
    isCOV19 = _isCOV19;
    setCssProperty(coinIcon, _isCOV19 ? "coin-icon-cov19" : "coin-icon-zcov19");
    refreshView();
...
void SendWidget::onContactMultiClicked(){
    if(focusedEntry) {
        ...
        CBitcoinAddress cov19Add = CBitcoinAddress(address.toStdString());
        if (walletModel->isMine(cov19Add)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(cov19Add.Get(), dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
