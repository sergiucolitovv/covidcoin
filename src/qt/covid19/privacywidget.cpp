...
#include "qt/covid19/privacywidget.h"
#include "qt/covid19/forms/ui_privacywidget.h"
#include "qt/covid19/qtutils.h"
#include "guiutil.h"
#include "qt/covid19/denomgenerationdialog.h"
#include "qt/covid19/txviewholder.h"
...
PrivacyWidget::PrivacyWidget(COVID19GUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zCOV19 anonymizes your COV19 by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zCOV19 or convert back to COV19"));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 COV19 ");
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zCOV19");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zCOV19");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zCOV19");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zCOV19");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zCOV19");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zCOV19");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zCOV19");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zCOV19");
    setCssProperty(ui->labelValueDenom5000, "text-body2");

    ui->layoutDenom->setVisible(false);

    // List
    ui->labelListHistory->setText(tr("Last zCOV19 Movements"));
    ...
    // Only Convert to COV19 enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzCOV19->setTitleClassAndText("btn-title-grey", "Total 0 zCOV19");
    ui->btnTotalzCOV19->setSubTitleClassAndText("text-subtitle", "Show denominations of zCOV19 owned.");
    ui->btnTotalzCOV19->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select COV19 outputs to mint into zCOV19.");
    ...
    ui->btnResetZerocoin->setTitleClassAndText("btn-title-grey", "Reset Spent zCOV19");
    ui->btnResetZerocoin->setSubTitleClassAndText("text-subtitle", "Reset zerocoin database.");

    connect(ui->btnTotalzCOV19, SIGNAL(clicked()), this, SLOT(onTotalZcov19Clicked()));
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint zCOV19");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of COV19 to mint into zCOV19"));
    }else{
        btnText = tr("Convert back to COV19");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zCOV19 to convert back into COV19"));
    }
...
void PrivacyWidget::onTotalZcov19Clicked(){
    bool isVisible = ui->layoutDenom->isVisible();
    if(!isVisible){
        ui->layoutDenom->setVisible(true);
        ui->btnTotalzCOV19->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzCOV19->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zCOV19 is currently undergoing maintenance"));
        return;
    }
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_zCOV19, true)){
        inform(tr("You need to unlock the wallet to be able to %1 zCOV19").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("zCOV19 minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackZcov19(
            value,
            selectedMints,
            mintChange,
            minimizeChange,
        ...
        // Spend succeed
        inform(tr("zCOV19 converted back to COV19"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zCOV19 </b>";
    ...
    ui->btnTotalzCOV19->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
