...
#include "qt/covid19/dashboardwidget.h"
#include "qt/covid19/forms/ui_dashboardwidget.h"
#include "qt/covid19/sendconfirmdialog.h"
#include "qt/covid19/txrow.h"
...
DashboardWidget::DashboardWidget(COVID19GUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of COV19 and zCOV19 staked."));
    setCssSubtitleScreen(ui->labelMessage);
    setCssProperty(ui->labelSquareCov19, "square-chart-cov19");
    setCssProperty(ui->labelSquarezCov19, "square-chart-zcov19");
    setCssProperty(ui->labelCov19, "text-chart-cov19");
    setCssProperty(ui->labelZcov19, "text-chart-zcov19");
    ...
    ui->labelAmountZcov19->setText("0 zCOV19");
    ui->labelAmountCov19->setText("0 COV19");
    setCssProperty(ui->labelAmountCov19, "text-stake-cov19-disable");
    setCssProperty(ui->labelAmountZcov19, "text-stake-zcov19-disable");
    ...
    ui->btnHowTo->setText(tr("How to get COV19 or zCOV19"));
...
void DashboardWidget::loadWalletModel(){
    ...
    // update the display unit, to not use the default ("COV19")
    updateDisplayUnit();
}
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy() {
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isCov19 = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZCOV19;
...
bool DashboardWidget::loadChartData(bool withMonthNames) {
    ...
    chartData->amountsByCache = getAmountBy(); // pair COV19, zCOV19
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            cov19 = (pair.first != 0) ? pair.first / 100000000 : 0;
            zcov19 = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalCov19 += pair.first;
            chartData->totalZcov19 += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesCov19.append(cov19);
        chartData->valueszCov19.append(zcov19);

        int max = std::max(cov19, zcov19);
...
void DashboardWidget::onChartRefreshed() {
    ...
    // init sets
    set0 = new QBarSet("COV19");
    set1 = new QBarSet("zCOV19");
    ...
    set0->append(chartData->valuesCov19);
    set1->append(chartData->valueszCov19);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalCov19 > 0 || chartData->totalZcov19 > 0) {
        setCssProperty(ui->labelAmountCov19, "text-stake-cov19");
        setCssProperty(ui->labelAmountZcov19, "text-stake-zcov19");
    } else {
        setCssProperty(ui->labelAmountCov19, "text-stake-cov19-disable");
        setCssProperty(ui->labelAmountZcov19, "text-stake-zcov19-disable");
    }
    forceUpdateStyle({ui->labelAmountCov19, ui->labelAmountZcov19});
    ui->labelAmountCov19->setText(GUIUtil::formatBalance(chartData->totalCov19, nDisplayUnit));
    ui->labelAmountZcov19->setText(GUIUtil::formatBalance(chartData->totalZcov19, nDisplayUnit, true));
