...
#include "qt/covid19/forms/ui_addresseswidget.h"
#include "qt/covid19/addresslabelrow.h"
#include "qt/covid19/addnewaddressdialog.h"
...
AddressesWidget::AddressesWidget(COVID19GUI* parent) :
    PWidget(parent),
    ui(new Ui::AddressesWidget)
{
    ...
    // Address
    ui->labelAddress->setText(tr("Enter COVID19 address"));
...
void AddressesWidget::onStoreContactClicked(){
    if (walletModel) {
        ...
        CBitcoinAddress cov19Add = CBitcoinAddress(address.toUtf8().constData());
        ...
        if (walletModel->updateAddressBookLabels(cov19Add.Get(), label.toUtf8().constData(),
                cov19Add.IsStakingAddress() ? AddressBook::AddressBookPurpose::COLD_STAKING_SEND : AddressBook::AddressBookPurpose::SEND)
                ) {
