...
bool GetHashProofOfStake(const CBlockIndex* pindexPrev, CStakeInput* stake, const unsigned int nTimeTx, const bool fVerify, uint256& hashProofOfStakeRet) {
    ...
    if (fVerify) {
        LogPrint("staking", "%s : nStakeModifier=%s (nStakeModifierHeight=%s)\n"
                "nTimeBlockFrom=%d\nssUniqueIDD=%s\n-->DATA=%s",
            __func__, HexStr(modifier_ss), ((stake->IsZCOV19()) ? "Not available" : std::to_string(stake->getStakeModifierHeight())),
...
bool ContextualCheckZerocoinStake(int nPreviousBlockHeight, CStakeInput* stake)
{
    if (nPreviousBlockHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s : zCOV19 stake block is less than allowed start height", __func__);

    if (CZCov19Stake* zCOV19 = dynamic_cast<CZCov19Stake*>(stake)) {
        CBlockIndex* pindexFrom = zCOV19->GetIndexFrom();
        if (!pindexFrom)
            return error("%s : failed to get index associated with zCOV19 stake checksum", __func__);

        int depth = (nPreviousBlockHeight + 1) - pindexFrom->nHeight;
        if (depth < Params().Zerocoin_RequiredStakeDepth())
            return error("%s : zCOV19 stake does not have required confirmation depth. Current height %d,  stakeInput height %d.", __func__, nPreviousBlockHeight, pindexFrom->nHeight);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zCOV19->GetValue()));
        if (nChecksum200 != zCOV19->GetChecksum())
            return error("%s : accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zCOV19->GetChecksum(), nChecksum200);
    } else {
...
bool initStakeInput(const CBlock& block, std::unique_ptr<CStakeInput>& stake, int nPreviousBlockHeight) {
    ...
    if (txin.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZCov19Stake(spend));

        if (!ContextualCheckZerocoinStake(nPreviousBlockHeight, stake.get()))
            return error("%s : staked zCOV19 fails context checks", __func__);
    } else {
        ...
        CCov19Stake* cov19Input = new CCov19Stake();
        cov19Input->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(cov19Input);
    }
    return true;
}
