...
from test_framework.test_framework import Covid19TestFramework
...
class ZerocoinSpendTest(Covid19TestFramework):
    ...
    def run_test(self):
        ...
        def check_balances(denom, zcov19_bal, cov19_bal):
            zcov19_bal -= denom
            assert_equal(self.nodes[2].getzerocoinbalance()['Total'], zcov19_bal)
            cov19_bal += denom
            wi = self.nodes[2].getwalletinfo()
            assert_equal(wi['balance'] + wi['immature_balance'], cov19_bal)
            return zcov19_bal, cov19_bal
        ...
        zcov19_balance = self.nodes[2].getzerocoinbalance()['Total']
        assert_equal(balance, DecimalAmt(13833.92))
        assert_equal(zcov19_balance, 6666)
        ...
        zcov19_balance, balance = check_balances(denom_0, zcov19_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        zcov19_balance, balance = check_balances(denom_2, zcov19_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        zcov19_balance, balance = check_balances(denom_3, zcov19_balance, balance)
        ...
        assert_raises_rpc_error(-26, "bad-txns-invalid-zcov19",
        ...
        _, _ = check_balances(denom_1, zcov19_balance, balance)
        self.log.info("--> VALID PUBLIC COIN SPEND (v3) PASSED")
