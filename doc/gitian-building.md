...
*Setup instructions for a gitian build of COVID19 Core using a VM or physical system.*
...
to the COVID19 GitHub release page.
...
- [Building COVID19 Core](#building-covid19-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/covid19-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/covid19-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/covid19-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/covid19-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/covid19-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/covid19-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/covid19-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@covid19.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/covid19-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/covid19-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/covid19-osx-*-build.assert
```
