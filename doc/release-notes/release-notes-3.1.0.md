COVID19 Core version *3.1.0* is now available from:  <https://github.com/covid19-project/covid19/releases>

This is a new major version release, including various bug fixes and performance improvements, as well as updated translations.

Please report bugs using the issue tracker at github: <https://github.com/covid19-project/covid19/issues>


Mandatory Update
==============

COVID19 Core v3.1.0 is a mandatory update for all users. This release contains new consensus rules and improvements that are not backwards compatible with older versions. Users will have a grace period of one week to update their clients before enforcement of this update is enabled.
...
zCOV19 Staking is here! zCOV19 staking will be activated on the morning of the 8th of May 2018. With the release of zCOV19 staking, there are effectively 2 versions of zCOV19, zCOV19 minted on the 3.0.6 COVID19 wallet or lower, and zCOV19 minted on COVID19 wallet version or higher. New features in this release will require the use of zCOV19 v2, zCOV19 minted on this wallet release 3.1.0 or later. If you currently hold zCOV19 v1 and wish to take advantage of zCOV19 staking and deterministic zCOV19, you will need to spend the zCOV19 v1 to yourself and remint zCOV19 v2.
Note: To find your zCOV19 version, click the privacy tab, then the zCOV19 Control button then expand the arrows next to the desired denomination.


### Deterministic zCOV19 Seed Keys

zCOV19 is now associated with a deterministic seed key. With this seed key, users are able to securely backup their zCOV19 outside of the wallet that the zCOV19 had been minted on. zCOV19 can also be transferred from wallet to wallet without the need of transferring the wallet data file.


### Updated zCOV19 minting

zCOV19 minting now only requires 1 further mint (down from 2) to mature. zCOV19 mints still require 20 confirmations.  Mints also require that the 'second' mint is at least two checkpoints deep in the chain (this was already the case, but the logic was not as precise).


### zCOV19 Search

Users will now have the ability to search the blockchain for a specific serial # to see if a zCOV19 denomination has been spent or not.



COV19/zCOV19 Staking and Masternode Rewards
--------------

### COV19, zCOV19 and Masternode Payment Schedule

To encourage the use of zCOV19 and increase the COVID19 zerocoin anonymity set, the COVID19 payment schedule has been changed to the following:

If a user staking zCOV19 wins the reward for their block, the following zCOV19 reward will be: 
- 3 zCOV19 (3 x 1 denominations) rewarded to the staker, 2 COV19 rewarded to the masternode owner and 1 COV19 available for the budget. This is a total block reward of 6 COV19, up from 5.

If a user staking COV19 wins the reward, the following amounts will be rewarded: 
- 2 COV19 to the COV19 staker, 3 COV19 to the Masternode owner and 1 COV19 available for the budget. This is a total block reward of 6 COV19, up from 5.


### Return change to sender when minting zCOV19

Previously, zCOV19 minting would send any change to a newly generated "change address". This has caused confusion among some users, and in some cases insufficient backups of the wallet. The wallet will now find the contributing address which contained the most COV19 and return the change from a zCOV19 mint to that address.
...
The visual layout of the COVID19 Qt wallet has undergone a near-complete overhaul.
...
-	Enable and disable the auto zCOV19 minting feature. This is enabled by default and the enablezeromint=0 setting in the covid19.conf file will overwrite the GUI option.
-	The percentage of autominted zCOV19 can now be set from 1 to 100, changed from 10 – 100.
-	The stake split threshold can now be set VIA the wallet options. This setting is an advanced feature for those wishing to remain staking regular COV19.
...
### Summary
 ...
 The COVID19 wallet can now have user selected directories for automatic backups of the wallet data file (wallet.dat). This can be set by adding the following lines to the covid19.conf file, found in the COVID19 data directory.
- backuppath = <directory / full path>
- zcov19backuppath = <directory / full path>
...
* If zCOV19 backup, auto generated name is `wallet-autozcov19backup.dat-<year>-<month>-<day>-<hour>-<minute>-<second>`
...
COVID19 Daemon & Client (RPC Changes)
--------------
...
This allows running covid19d without having to do any manual configuration.
...
Search the zerocoin database for a zerocoinspend transaction that contains the given serial. This will be a helpful tool for the COVID19 support group, which often times sees users say "I didn't spend that zCOV19". This RPC call allows for support to grab the serial, and then find the spend tx on the chain.
...
As voted on by the COVID19 masternodes, the monthly budget available to be utilised has been increased to 42,000 COV19 / month. This COV19 only has the opportunity to be raised once per month (paid to winning proposals) with any unused COV19 not created by the blockchain.

### Budget Finalization Fee

The COVID19 finalization fee for successful proposals has now been reduced, this fee is now 5 COV19 down from 50 COV19. The total fee outlay for a successful proposal is now a total of 55 COV19.
...
-	Enforced v1 zCOV19 spends to require a security level of 100
-	Updates to zCOV19 spends to avoid segfaults
-	Updates to configuration will now reflect on the privacy tab
-	Fixed a  bug that would not start masternodes from the COVID19-Qt masternodes tab
-	Updated COVID19-Qt tooltips
...
Several critical security flaws in the zerocoin protocol and COVID19's zerocoin implementation have been patched. Enough has changed that new zerocoins are distinct from old zerocoins, and have been labelled as *version 2*. When using the zCOV19 Control dialog in the QT wallet, a user is able to see zCOV19 marked as version 1 or 2.

zPoS (zCOV19 staking)
--------------

Once a zCOV19 has over 200 confirmations it becomes available to stake. Staking zCOV19 will consume the exact zerocoin that is staked and replace it with a freshly minted zerocoin of the same denomination as well as a reward of three 1 denomination zCOV19. So for example if a 1,000 zCOV19 denomination is staked, the protocol replaces that with a fresh 1,000 denomination and three1 denomination zCOV19s.

Secure Spending
--------------

Version 1 zerocoins, as implemented by [Miers et. al](http://zerocoin.org/media/pdf/ZerocoinOakland.pdf), allow for something we describe as *serial trolling*. Spending zerocoins requires that the spender reveal their serial number associated with the zerocoin, and in turn that serial number is used to check for double spending. There is a fringe situation (which is very unlikely to happen within COVID19's zerocoin implementation due to delayed coin accumulation) where the spender sends the spending transaction, but the transaction does not immediately make it into the blockchain and remains in the mempool for a long enough duration that a *troll* has enough time to see the spender's serial number, mint a new zerocoin with the same serial number, and spend the new zerocoin before the original spender's transaction becomes confirmed. If the timing of this fringe situation worked, then the original spender's coin would be seen as invalid because the troll was able to have the serial recorded into the blockchain first, thus making the original spender's serial appear as a double spend.
...
Zerocoins, or zCOV19, are now deterministically generated using a unique 256 bit seed. Each wallet will generate a new seed on its first run. The deterministic seed is used to generate a string of zCOV19 that can be recalculated at any time using the seed. Deterministic zCOV19 allows for users to backup all of their future zCOV19 by simply recording their seed and keeping it in a safe place (similar to backing up a private key for COV19). The zCOV19 seed needs to remain in the wallet in order to spend the zCOV19 after it is generated, if the seed is changed then the coins will not be spendable because the wallet will not have the ability to regenerate all of the private zCOV19 data from the seed. It is important that users record & backup their seed after their first run of the wallet. If the wallet is locked during the first run, then the seed will be generated the first time the wallet is unlocked.

Zerocoin Modulus
--------------

COVID19's zerocoin implementation used the same code from the ZCoin project to import the modulus use for the zerocoin protocol. The chosen modulus is the 2048 bit RSA number created for the RSA factoring challenge. The ZCoin project's implementation (which COVID19 used) improperly imported the modulus into the code. This flaw was discovered by user GOAT from the [Civitas Project](https://github.com/eastcoastcrypto/Civitas/), and reported to COVID19 using the bug bounty program. The modulus is now correctly imported and COVID19's accumulators have been changed to use the new proper modulus.
...
### Core Features
 - #582 `cb1632520` [Core] zCOV19 v2: zPoS dzCOV19 ezCOV19 external backup and budget fixes (too many to list)
...
### GUI
 - #572 `d9b23fe60` [Qt] Refresh zCOV19 balance after resetting mints or spends (warrows)
 ...
 - #506 `ae72bf4e2` [Qt] Autoscroll to end of zCOV19 status output (Mrs-X)
 ...
 - #467 `7099831bd` [Qt] Clear PrivacyDialog "zCov19 Selected" labels after sending. (presstab)
...
### RPC/REST
 ...
 - #543 `e4522ff07` [RPC] Segfault covid19-cli getinfo while loading block index (Mrs-X)
...
### Wallet
 - #570 `8c8350b59` [Wallet] Add a check on zCOV19 spend to avoid a segfault (warrows)
 ...
 - #448 `222ef6e6b` [Wallet] Return change to sender when minting zCOV19. (presstab)
 - #445 `fc570fc1e` [Wallet] Only require 1 mint to be added before spending zCOV19. (presstab)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/covid19-project-translations/), the QA team during Testing and the Node hosts supporting our Testnet.
