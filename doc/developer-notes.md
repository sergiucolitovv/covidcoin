...
- [Development guidelines](#development-guidelines)
    - [General COVID19 Core](#general-covid19-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
COVID19 Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play COV19s (tCOV19)" on the test network, if you
...
COVID19 Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/covid19-project/covid19/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_covid19
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_covid19 --log_level=test_suite
$ valgrind -v --leak-check=full src/covid19d -printtoconsole
```
...
# A coverage report will now be accessible at `./test_covid19.coverage/index.html`.
```
...
- BitcoinMiner : Generates COV19s (if wallet is enabled).
...
be sent upstream but bugfixes may also be prudent to PR against COVID19 Core so that they can be integrated
...
file descriptors it will cause problems with COVID19's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:COVID19-Project/COVID19.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `covid19-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `covid19-cli` and the GUI debug console use this table to determine how to
